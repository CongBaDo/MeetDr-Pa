...
class InformationAccountActivity : BaseActivity(),View.OnFocusChangeListener,View.OnClickListener,DatePickerDialog.OnDateSetListener {
    ...
    override fun onFocusChange(v: View?, hasFocus: Boolean) {
        ...
        when(id)
        {
            R.id.edtFirstName -> {
                if(!hasFocus){
                    if(edtFirstName.text.trim().toString() == "" || edtFirstName.text.toString() == null)
                    {
                        input_FirstName.isErrorEnabled = true
                        input_FirstName.error = resources.getString(com.viiam.mvvp.R.string.banchuanhapmucnay)
            ...
            R.id.edtLastName -> {
                if(!hasFocus){
                    if(edtLastName.text.trim().toString() == "" || edtLastName.text.toString() == null)
                    {
                        input_LastName.isErrorEnabled = true
                        input_LastName.error = resources.getString(com.viiam.mvvp.R.string.banchuanhapmucnay)
            ...
            R.id.edtDiaChi ->{
                if(!hasFocus){
                    if(edtDiaChi.text.trim().toString() == "" || edtDiaChi.text.toString() == null)
                    {
                        input_DiaChi.isErrorEnabled = true
                        input_DiaChi.error = resources.getString(com.viiam.mvvp.R.string.banchuanhapmucnay)
                        CHECK_DIACHI = false
                    }
                    else{
            ...
            R.id.edtNamSinh -> {
                if(!hasFocus){
                    if(edtNamSinh.text.trim().toString() == "" || edtNamSinh.text.toString() == null)
                    {
                        input_NamSinh.isErrorEnabled = true
                        input_NamSinh.error = resources.getString(com.viiam.mvvp.R.string.banchuanhapmucnay)
                        CHECK_NAMSINH = false
                    }
                    else{
            ...
            R.id.edtEmail ->{
                if(!hasFocus){
                    if(edtEmail.text.trim().toString() == "" || edtEmail.text.toString() == null)
                    {
                        ...
                        input_Email.error = resources.getString(com.viiam.mvvp.R.string.banchuanhapmucnay)
                        CHECK_EMAIL = false
                    }
                    else{
